...
- Use `--debug` for app diagnostics; keep `--debug-ws` OFF unless you need raw websocket event traces (very noisy).
...
- If data disappears right after a market rollover, update to latest version: resubscription now explicitly unsubscribes old tokens and subscribes new ones.
