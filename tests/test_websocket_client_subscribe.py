...
def test_replace_subscription_unsubscribes_old_assets_first():
    ws = _make_connected_ws()
    ...
    first_payload = json.loads(ws._ws.sent[0])  # type: ignore[attr-defined]
    second_payload = json.loads(ws._ws.sent[1])  # type: ignore[attr-defined]

    assert first_payload["type"] == "market"
    assert first_payload["operation"] == "unsubscribe"
    assert set(first_payload["assets_ids"]) == {"old1", "old2"}

    assert second_payload == {"assets_ids": ["new1", "new2"], "type": "market", "operation": "subscribe"}
